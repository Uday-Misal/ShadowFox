import teethCleaning from "../assets/Teeth Cleaning.png";
import toothFilling from "../assets/Tooth-Filling.jpg";
import toothExtraction from "../assets/Teeth Extraction.png";
import artificialDenture from "../assets/Artificial Complete Denture.png";
import implants from "../assets/Impants.png";
import laserSurgery from "../assets/Laser-Surgery.png";
import orthodontic from "../assets/Orthodonic-Treatment.png";
import pediatric from "../assets/Pediatric-dentistry.png";
import veneer from "../assets/Veneer Treatment.jpg";
import aligner from "../assets/Aligner.png";
import wisdom from "../assets/Wisdom Teeth Removal.png";
import root from "../assets/Root Canal Treatment.png";
import flap from "../assets/Flap-surgery.png";
import fixed from "../assets/Fixed-surgery.png";
import bleaching from "../assets/Bleaching.png";

export const allTreatments = [
  {
    title: "Teeth Cleaning & Scaling",
    image: teethCleaning,
    desc: "At Sakthi Dental Clinic, we provide advanced non-surgical gum care using modern LASER technology for precise and effective treatment. Scaling is recommended for moderate gum issues, while severe cases may require deep cleaning such as root planing with LASER support. This approach restores gum health and prepares patients for any further treatment if needed."
  },
  {
    title: "Tooth Filling",
    image: toothFilling,
    desc: "At Sakthi Dental Clinic, we offer expert tooth filling services to restore and protect damaged teeth caused by cavities, trauma, or wear. Using high-quality materials and advanced techniques, we seal gaps effectively to prevent future decay while preserving your natural tooth structure. Our treatments enhance both function and appearance for a healthier, confident smile."
  },
  {
    title: "Tooth Extraction",
    image: toothExtraction,
    desc: "When a tooth cannot be saved, Sakthi Dental Clinic ensures safe and comfortable tooth extraction using gentle techniques. Our experienced team carefully evaluates all options, explains each step, and focuses on minimizing discomfort. We provide a calm, stress-free experience while supporting you through a smooth recovery."
  },
  {
    title: "Artificial Complete Denture",
    image: artificialDenture,
    desc: "At Sakthi Dental Clinic, we design high-quality complete dentures to restore function and appearance for patients with missing teeth."
  },
  {
    title: "Dental Implants",
    image: implants,
    desc: "Dental implants at Sakthi Dental Clinic provide a reliable solution for replacing missing teeth."
  },
  {
    title: "Laser Dentistry",
    image: laserSurgery,
    desc: "We use advanced laser technology for precise, minimally invasive dental treatments."
  },
  {
    title: "Root Canal Therapy",
    image: root,
    desc: "Precise root canal treatment to remove infection and preserve your natural tooth."
  },
  {
    title: "Wisdom Tooth Extraction",
    image: wisdom,
    desc: "Gentle and effective wisdom tooth removal using modern techniques."
  },
  {
    title: "Fixed Partial Denture (Bridge)",
    image: fixed,
    desc: "Custom-made fixed partial dentures to replace missing teeth."
  },
  {
    title: "Teeth Whitening (Bleaching)",
    image: bleaching,
    desc: "Professional teeth whitening to restore natural brightness."
  },
  {
    title: "Veneers",
    image: veneer,
    desc: "Custom veneers to enhance your smile."
  },
  {
    title: "Pediatric Dentistry",
    image: pediatric,
    desc: "Gentle and comprehensive dental care for children."
  },
  {
    title: "Flap Surgery",
    image: flap,
    desc: "Advanced gum treatment to remove deep plaque and bacteria."
  },
  {
    title: "Orthodontic Braces",
    image: orthodontic,
    desc: "Customized orthodontic treatments for perfect alignment."
  },
  {
    title: "Clear Aligners",
    image: aligner,
    desc: "Discreet and removable clear aligners."
  }
];
